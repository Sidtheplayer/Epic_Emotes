...
public class JointTrackedEntityEffect extends EntityEffectExecutor {
    ...
    public void updateFXObjectFrame(IFXObject fxObject, float partialTicks) {
        ...
        if (cachedPatch == null) {
            ...
            if (cachedPatch == null) {
                EpicEmotez.LOGGER.warn("[JointTrackedEntityEffect] No EpicFight patch on {}, disabling.", living);
                rotationFailed = true;
                return;
            }
        }
        ...
        long currentTick;
        currentTick = entity.level().getGameTime();
    ...
    private void sampleTickBoundaries() {
        try {
            ...
            EpicEmotez.LOGGER.error("[JointTrackedEntityEffect] Tick sampling failed, disabling rotation: {}", e.getMessage());
            rotationFailed = true;
        }
    }
}